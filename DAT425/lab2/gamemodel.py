...
class Game:
    ...
    def getOtherPlayer(self):
        otherPlayer = (1 if self.currentPlayerNumber == 0 else 0)
    ...
    def nextPlayer(self):
        self.currentPlayerNumber = (1 if self.currentPlayerNumber == 0 else 0)
...
class Player:
    ...
    def fire(self, angle, velocity):
        ...
        # Important because we want both users to use same angles
        if self.xPos > 0:
            angle = 180 - angle
            
        self.proj = Projectile(angle, velocity, self.game.getCurrentWind(), 
            self.xPos, self.game.cannonSize/2, -110, 110)
    ...
    def projectileDistance(self, proj):
        ...
        if abs(distance) <=  (self.game.ballSize + self.game.cannonSize/2):
            return 0

        #Distance minus half cannon (because it is in the center)
        realDistance = distance - (self.game.ballSize + self.game.cannonSize/2)
...
class Projectile:
    ...
    def isMoving(self):
        #print("moving", "0 <", self.getY(), "&&", self.xLower," <", self.getX()," < ", self.xUpper)
